...
```bash
curl -s -X PUT https://localhost:9996/demo-basic/TableName/main-counter \
...
```bash
curl -s https://localhost:9996/demo-basic/TableName/main-counter
```
...
```bash
curl -s https://localhost:9996/demo-basic/greeting
```
...
```bash
curl -s "https://localhost:9996/demo-basic/TableName?limit=10"
```
...
curl -s --max-time 30 "https://localhost:9996/demo-basic/TableName?stream=sse"
```
...
Browser / curl / AI Agent
    ...
    +-- GET/PUT /TableName/{id} ----> Schema-driven REST (RocksDB)
    +-- GET /greeting --------------> Custom Rust resource (plugin)
    +-- GET /TableName?stream=sse --> Real-time SSE stream
...
|----------|--------|-------------|
| `/demo-basic/TableName` | GET | List all records (with `?limit=N`) |
| `/demo-basic/TableName` | POST | Create a new record |
| `/demo-basic/TableName/{id}` | GET | Read a single record |
| `/demo-basic/TableName/{id}` | PUT | Update a record |
| `/demo-basic/TableName/{id}` | DELETE | Delete a record |
| `/demo-basic/TableName?stream=sse` | GET | Real-time change stream |
...
This compiles to a native Rust plugin that responds at `/demo-basic/greeting`. The `resource!()` macro supports all HTTP methods (`get`, `post`, `put`, `delete`) and has access to the full `Request` and `ExtensionContext` when needed:
...
- **Counter panel** -- increment/decrement buttons that call `PUT /TableName/{id}` and display the current count
- **Greeting panel** -- a button that calls `GET /greeting` and displays the JSON response
...
| Key | Purpose |
|-----|---------|
| `schemas` | List of GraphQL schema files. Each `@table` type becomes a RocksDB-backed table with auto-generated REST/SSE/MQTT/MCP endpoints. |
| `resources` | Glob pattern for Rust resource files. Each file is compiled into a native plugin and routed by struct name. |
| `static_files.path` | Directory containing built static assets (output of `npm run build`). |
| `static_files.spa` | Enables SPA routing -- unknown paths serve `index.html` with a 200 status. |
| `static_files.build` | Auto-build configuration. Yeti runs the command from `sourceDir` if `path` is empty or missing. |
...
|----------|--------------|-------|
| `GET /TableName` | No | Public read via schema directive |
| `GET /TableName/{id}` | No | Public read via schema directive |
| `PUT /TableName/{id}` | No | Public update via schema directive |
| `POST /TableName` | Yes | Create not listed in public access |
| `DELETE /TableName/{id}` | Yes | Delete not listed in public access |
| `GET /greeting` | No | Custom resources are public by default in dev |
